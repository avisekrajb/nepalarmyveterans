...
const TaskProgramManager = () => {
  ...
  const loadItems = async () => {
    try {
      ...
      toast.error('Failed to load task programs');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editing) {
        const { data } = await taskProgramAPI.updateTaskProgram(editing, { ...formData, order: formData.order || 0 });
        setItems(items.map(item => item._id === editing ? data : item));
        toast.success('Task program updated successfully');
      } else {
        const { data } = await taskProgramAPI.createTaskProgram({ ...formData, order: formData.order || 0 });
        setItems([...items, data]);
        toast.success('Task program created successfully');
      }
      resetForm();
    } catch (error) {
  ...
  const deleteItem = async (id) => {
    if (!window.confirm('Delete this task program?')) return;
    try {
      await taskProgramAPI.deleteTaskProgram(id);
      setItems(items.filter(item => item._id !== id));
      toast.success('Task program deleted');
    } catch (error) {
      toast.error('Failed to delete task program');
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-bold text-army">Task Program Management</h2>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden">
            ...
            {items.map((item, idx) => (
              <tr key={item._id}>
                ...
                <td className="px-6 py-4 text-gray-600 max-w-md">
                  ...
                  {item.descriptionNe && <div className="text-xs text-gray-400">{item.descriptionNe}</div>}
                </td>
        ...
        {items.length === 0 && (
          <p className="text-center text-gray-500 py-8">No task programs added yet</p>
        )}
      </div>
    </div>
  );
...
export default TaskProgramManager;